...
for pid in [0]:
    ...
    imp_mean.fit(this_pid)
    imp_median.fit(this_pid)
    imp_const.fit(this_pid)
    this_pid_impmean=imp_mean.transform(this_pid)
    this_pid_impmedian = imp_median.transform(this_pid)
    this_pid_impconst = imp_const.transform(this_pid)
    ...
    print(md_list_impmean[:,0:5,pid])
    md_list_impmedian[:, :, pid] = this_pid_impmedian
    md_list_impconst[:, :, pid] = this_pid_impconst

print(md_list_impmean[:,0:5,0])

print("Imputing finished, starting to write the multidimensional lists to csv's...")
print_elapsed_time(totaltime_start)


md_list2pdSeries(md_list_impmean,fulldata.columns.tolist(),"mean")
print("Finished writing mean-file...")
print_elapsed_time(totaltime_start)
md_list2pdSeries(md_list_impmedian,fulldata.columns.tolist(),"median")
print("Finished writing median-file...")
print_elapsed_time(totaltime_start)
md_list2pdSeries(md_list_impconst,fulldata.columns.tolist(),"constant_0")
