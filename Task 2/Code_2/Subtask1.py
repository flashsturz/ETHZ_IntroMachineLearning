...
def pipeCV(pipe, train_data, train_labels, n_cv):
    cv_results = cross_validate(pipe, train_data, train_labels, scoring='roc_auc', return_train_score=True,
...
def solveSubtask1(train_data_imputed_reduced_pd, test_data_imputed_reduced_pd, train_labels_task1_pd):
    ...
    print(
        "TASK 1: Fit Multilabel Classifier starts. Set up Pipeline and Cross Validate with MLPClassifier (solver= adam, activation= logistic, max_iter = 150) and n_cv = 5")
    ...
    pipe_1_mlp = make_pipeline(StandardScaler(),
                               MLPClassifier(solver='adam', activation='logistic', random_state=42, max_iter=150))
