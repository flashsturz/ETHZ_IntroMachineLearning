...
def iterativeImpute(train_data_path, test_data_path, use_gradients):
    ...
    if data_import_active or not (os.path.isfile('../Data_2/train_features_iterImp.csv') and os.path.isfile('../Data_2/train_features_reduced.csv')):
...
#        if not os.path.isfile('../Data_2/train_features_iterImp_reduced.csv'):
        ...
        np.savetxt('../Data_2/train_features_iterImp_reduced.csv', train_data_reduced, fmt=('%.3f'), delimiter=',', comments='',)
        np.savetxt('../Data_2/test_features_iterImp_reduced.csv', test_data_reduced, fmt=('%.3f'), delimiter=',', comments='',)
#        else:
...
#            test_data_reduced = np.genfromtxt('../Data_2/test_features_iterImp_reduced.csv', dtype=float, delimiter=',')
        ...
        return train_data_reduced_pd, test_data_reduced_pd, train_data_imp_pd, test_data_imp_pd
...
#        if not os.path.isfile('../Data_2/train_features_iterImp_reduced_withGrad.csv'):
        ...
        np.savetxt('../Data_2/train_features_iterImp_reduced_withGrad.csv', train_data_reduced_withGrad, fmt=('%.3f'), delimiter=',', comments='',)
        np.savetxt('../Data_2/test_features_iterImp_reduced_withGrad.csv', test_data_reduced_withGrad, fmt=('%.3f'), delimiter=',', comments='',)
#        else:
...
#            test_data_reduced_withGrad = np.genfromtxt('../Data_2/test_features_iterImp_reduced_withGrad.csv', dtype=float, delimiter=',')
        ...
        train_data_reduced_withGrad_pd = pd.DataFrame(train_data_reduced_withGrad, columns = [*train_data_frame.columns, *gradient_labels])
        test_data_reduced_withGrad_pd = pd.DataFrame(test_data_reduced_withGrad, columns = [*test_data_frame.columns, *gradient_labels])
